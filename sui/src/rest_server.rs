...
// discount coupon
use sui::coupon_utils::{CAMPAIGN_ID, gen_address_and_mnemonic, send_coupon_email};
...
// Validate if it is a Coin object
async fn is_coin_object(state: &ServerState, object_id: ObjectID) -> Result<bool, HttpError> {
    let object = match get_object_info(state, object_id).await {
        Ok((_, object, _)) => object,
        Err(error) => {
            return Err(error);
    ...
    Ok(obj_type == "0x2::Coin::Coin<0x2::SUI::SUI>".to_string())
}
